...
- **Phase 23 — Experience Modes.** Five visual worlds over the same substrate: *Observatory* (instrument), *Field Journal* (notebook), *Daily Edition* (broadsheet), *Workbench* (shop floor), *Quiet Atrium* (pinboard) — plus an *Adaptive* mode that resolves deterministically from artifact count, active-session count, and high-priority ratio. Each mode ships its own design tokens (light + dark variants), home surface, artifact-detail surface, sessions surface, agent-presence motion, and per-component CSS reinterpretation. The whole agent loop, schema, parser, and managed-session flow are unchanged — only the rendering and copy adapt.

Detail in [CHANGELOG.md](../CHANGELOG.md#unreleased).

**What `main` looks like today:** scaffold + 23 phases of substrate + five visual experiences + 54 artifact components + cron triggers + session continuity + universal reply + session lifecycle + ambient sources / reflexes / living artifacts + Vocabulary v2 thinking primitives + OSS scaffolding (LICENSE, CI matrix, CodeQL, Dependabot). Onboarding plays a 5-step cinematic. The agent has memory across turns within a local session.
...
| **23** | **Experience Modes** — five visual worlds (Observatory / Field Journal / Daily Edition / Workbench / Quiet Atrium) + adaptive mode. Per-experience design tokens with light/dark variants, home surfaces, artifact-detail surfaces, sessions surfaces, agent-presence motion × state, and per-component CSS reinterpretation. Same agent loop / schema / parser / managed-session flow underneath. |
